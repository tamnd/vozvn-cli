...
describes what users get. Pick a format with `-o`, or let vozvn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vozvn <command> -o table   # aligned columns for reading
vozvn <command> -o jsonl   # one JSON object per line, for piping
vozvn <command> -o json    # a single JSON array
vozvn <command> -o csv     # spreadsheet friendly
vozvn <command> -o tsv     # tab-separated
vozvn <command> -o url     # just the URL column
vozvn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vozvn <command> --fields id,title,url
```
...
```bash
vozvn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vozvn <command>            # a table, because this is a terminal
vozvn <command> | wc -l    # JSONL, because this is a pipe
```
